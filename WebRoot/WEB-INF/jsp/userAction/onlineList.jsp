	...
	<div class="easyui-panel" title="查询条件" style="width:900px;padding:5px;">
	    <table id="searchTable">
	        <tr>
	         ...
	         <tr>
	            <td>开始时间:</td>
	            <td><input name="startTime" id="email" class="easyui-datebox" /></td>
	            <td>结束时间:</td>
	            <td><input name="endTime" id="email" class="easyui-datebox" /></td>
	        </tr>
	        <tr>
...
<script type="text/javascript">
	$(function() {
		//定义表格上的工具条
		var toolbar = [ 
			{text : '强制下线', iconCls : 'icon-cancel', handler : forceLogout}//初始化用户密码，支持批量操作
		];

		//加载datagrid数据表格
		$('#dategrid').datagrid({
			url : 'user_onlineList.action',
			columns : [ [ 
	...
	function forceLogout(){
		...
		//如果没有选中的行，提示错误并返回
		if($checkedRows.length == 0){
			$.messager.alert('强制下线','请选择要操作的行!','error');//不要用alert,用$.messager.alert，error表示这是个错误提示
			return false;
		} else {
			$.messager.confirm('强制下线', '确定将选中的用户强制下线吗？', function(r){//确认框
	        	if (r){//如果确认操作，就获取到要操作的行的id，并个发送初始化密码请求
	        		var forceLogoutIds = new Array();//要被强制下线的id数组
	    			
	        		//遍历将选中行的id加入deleteIds数组
	    			$.each($checkedRows, function(key, val) {
	    				forceLogoutIds[key] = val.id;
	    			});
	        		var postData = {'forceLogoutIds': forceLogoutIds};
	        		//ajax 请求删除选中的行
	    			$.ajax({
	    				url: 'user_forceLogout.action',
	    				type: 'POST',
	    				...
	    				async: true,//默认为true，代表异步请求；如果为false，代表同步请求
	    				success: function(data){
	    					$.messager.alert("强制用户下线",data,'info',function(){
	    						location.reload(true);//显示信息后的回调函数：重新加载原页面
	    					});
	    				}
	    			});//end of $.ajax
